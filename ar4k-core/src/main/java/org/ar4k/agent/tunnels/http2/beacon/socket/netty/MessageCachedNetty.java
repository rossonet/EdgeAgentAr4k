...
import org.ar4k.agent.tunnels.http2.grpc.beacon.TunnelMessage.Builder;
...
final class MessageCachedNetty implements Serializable {
	...
	private final transient BeaconNettyNetworkReceiver networkReceiver;
	...
	private final transient BeaconNettyNetworkTunnel tunnel;
	...
	MessageCachedNetty(final MessageCachedType messageCachedType, final BeaconNettyNetworkReceiver networkReceiver,
			final BeaconNettyNetworkTunnel tunnel, final ChannelHandlerContext ctx, final NetworkMode myRoleMode,
	...
	synchronized boolean softChek() {
		...
		if (lastRetry == 0 || (lastRetry + BeaconNettyNetworkTunnel.DELAY_SOFT_CHECK < time)) {
			lastRetry = time;
	...
	private void sendToNetworkServer(final byte[] primitiveBytes, byte[] decompressedBytes)
		...
		if (!isCompleted()) {
			networkReceiver.getOrCreateServerSocketChannel(serialId)
					...
					.get(BeaconNettyNetworkTunnel.LAST_MESSAGE_FROM_BEACON_SERVER_TIMEOUT, TimeUnit.MILLISECONDS);
			end();
		}
	}
		...
		if (!isCompleted()) {
			if (isValidToRetry()) {
				...
				logger.warn("for message " + messageId + " received " + retry + " reset but the limit is "
						+ BeaconNettyNetworkTunnel.RETRY_LIMIT);
			}
		} else {
	...
	boolean isValidToRetry() {
		return retry < BeaconNettyNetworkTunnel.RETRY_LIMIT;
	}
