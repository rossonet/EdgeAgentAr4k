...
public class ConfigLoadingWeb {
	...
	public void checkConfigWeb() throws InterruptedException, IOException {
		...
		System.out.println("CONFIG -> " + ConfigHelper.toYaml(edgeAgentCore.getRuntimeConfig()));
		assertTrue(checkString.equals(edgeAgentCore.getRuntimeConfig().author));
		System.out.println(
				"NOTE 0 -> " + ((BeaconServiceConfig) edgeAgentCore.getRuntimeConfig().pots.toArray()[0]).note);
		assertTrue(
				checkString.equals(((BeaconServiceConfig) edgeAgentCore.getRuntimeConfig().pots.toArray()[0]).note));
		System.out.println(
				"NOTE 1 -> " + ((BeaconServiceConfig) edgeAgentCore.getRuntimeConfig().pots.toArray()[1]).note);
		assertTrue(
				checkString.equals(((BeaconServiceConfig) edgeAgentCore.getRuntimeConfig().pots.toArray()[1]).note));
	}
