...
public class GrpcBeaconTests {
	...
	public void setUp() throws Exception {
		...
		while (!edgeAgentCore.getState().equals(HomunculusStates.STAMINAL)) {
			System.out.println("waiting anima, actual state: " + edgeAgentCore.getState());
			Thread.sleep(1500L);
		}
		Thread.sleep(1500L);
		System.out.println("Beacon server starting");
		server = new BeaconServerBuilder().setHomunculus(edgeAgentCore).setPort(port)
				.setStringDiscovery("AR4K-BEACON-" + UUID.randomUUID().toString())
				.setBroadcastAddress("255.255.255.255").setAcceptCerts(true)
				.setAliasBeaconServerInKeystore(edgeAgentCore.getMyAliasCertInKeystore()).build();
		server.start();
	...
	public void implementTest() {
		try {
			Thread.sleep(6000L);
			System.out.println("looking for alias " + edgeAgentCore.getMyAliasCertInKeystore());
			System.out.println("in keystore certificates " + edgeAgentCore.getMyIdentityKeystore().listCertificate());
	...
	public void implementTestClass() throws UnrecoverableKeyException {
		try {
			Thread.sleep(2000L);
			client = new BeaconClientBuilder().setHomunculus(edgeAgentCore).setPort(port)
					.setRpcConversation(rpcConversation).setHost("localhost").setDiscoveryPort(0)
					.setAliasBeaconClientInKeystore(edgeAgentCore.getMyAliasCertInKeystore()).build();
			Thread.sleep(10000L);
