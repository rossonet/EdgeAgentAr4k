...
public class AuthenticationAndRpcTests {
	...
	public void authenticationTest() {
		while (edgeAgentCore.getState() != null && edgeAgentCore.getState().equals(HomunculusStates.INIT)) {
			try {
				Thread.sleep(200L);
		...
		printBeans();
		checkAuthentication();
		edgeAgentCore.loginAgent("admin", "a4c8ff551a", null);
	...
	public void baseSessionManagerTest() {
		String sessionId = edgeAgentCore.loginAgent("admin", "a4c8ff551a", null);
		checkAuthentication();
		assertEquals("admin", SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
		RpcConversation rpc = (RpcConversation) edgeAgentCore.getRpc(sessionId);
	...
	public void destroySessionTest() {
		...
		checkAuthentication();
		assertEquals("admin", SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString());
		edgeAgentCore.terminateSession(sessionId);
