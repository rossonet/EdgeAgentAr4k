package org.ar4k.agent.tunnels.proxy;
...
public class ProxySocketFactory extends SocketFactory {
  ...
  public ProxySocketFactory(ProxyConfig configuration) {
  ...
  private Socket getProxySocket() throws IOException {
    ...
    if (config.protocol == "DIRECT") {
      proxy = new Proxy(Proxy.Type.DIRECT, new InetSocketAddress(config.ipHost, config.port));
    }
