...
public class OpcUaNamespace extends ManagedNamespace {
  ...
  protected void onStartup() {
    ...
    if (serverNode instanceof ServerNode) {
      ...
      getServer().getScheduledExecutorService().scheduleAtFixedRate(() -> {
        try {
          BaseEventNode eventNode = getServer().getEventFactory().createEvent(newNodeId(UUID.randomUUID()),
  ...
  private void addWriteOnlyNodes(UaFolderNode rootNode) {
    ...
    UaVariableNode node = new UaVariableNode.UaVariableNodeBuilder(getNodeContext())
        ...
        .setUserAccessLevel(ubyte(AccessLevel.getMask(AccessLevel.WRITE_ONLY))).setBrowseName(newQualifiedName(name))
  ...
  private void addAdminReadableNodes(UaFolderNode rootNode) {
    ...
    UaVariableNode node = new UaVariableNode.UaVariableNodeBuilder(getNodeContext())
        ...
        .setAccessLevel(ubyte(AccessLevel.getMask(AccessLevel.READ_WRITE))).setBrowseName(newQualifiedName(name))
        .setDisplayName(LocalizedText.english(name)).setDataType(Identifiers.String)
        .setTypeDefinition(Identifiers.BaseDataVariableType).build();
  ...
  private void addDataAccessNodes(UaFolderNode rootNode) {
    ...
    // AnalogItemType node
    try {
      AnalogItemNode node = (AnalogItemNode) getNodeFactory().createNode(newNodeId("HelloWorld/DataAccess/AnalogValue"),
          Identifiers.AnalogItemType, true);
  ...
  private void addCustomObjectTypeAndInstance(UaFolderNode rootFolder) {
    ...
    UaVariableNode foo = UaVariableNode.builder(getNodeContext()).setNodeId(newNodeId("ObjectTypes/MyObjectType.Foo"))
        .setAccessLevel(ubyte(AccessLevel.getMask(AccessLevel.READ_WRITE))).setBrowseName(newQualifiedName("Foo"))
    ...
    UaVariableNode bar = UaVariableNode.builder(getNodeContext()).setNodeId(newNodeId("ObjectTypes/MyObjectType.Bar"))
        .setAccessLevel(ubyte(AccessLevel.getMask(AccessLevel.READ_WRITE))).setBrowseName(newQualifiedName("Bar"))
    ...
    try {
      UaObjectNode myObject = (UaObjectNode) getNodeFactory().createNode(newNodeId("HelloWorld/MyObject"),
          objectTypeNode.getNodeId(), false);
  ...
  private void addCustomDataTypeVariable(UaFolderNode rootFolder) {
    ...
    UaDataTypeNode dataTypeNode = new UaDataTypeNode(getNodeContext(), dataTypeId, newQualifiedName("CustomDataType"),
        LocalizedText.english("CustomDataType"), LocalizedText.english("CustomDataType"), uint(0), uint(0), false);

    getNodeManager().addNode(dataTypeNode);

    // Inverse ref to Structure
    dataTypeNode
        .addReference(new Reference(dataTypeId, Identifiers.HasSubtype, Identifiers.Structure.expanded(), false));

    // Forward ref from Structure
    Optional<UaDataTypeNode> structureDataTypeNode = getNodeManager().getNode(Identifiers.Structure)
        .map(UaDataTypeNode.class::cast);

    structureDataTypeNode.ifPresent(node -> node
        .addReference(new Reference(node.getNodeId(), Identifiers.HasSubtype, dataTypeId.expanded(), true)));

    // TODO this should probably get a node and a HasEncoding reference from
    // dataTypeNode...
    ...
    getServer().getDataTypeManager().registerCodec(binaryEncodingId, new CustomDataType.Codec().asBinaryCodec());
    ...
    UaVariableNode customDataTypeVariable = UaVariableNode.builder(getNodeContext())
        ...
        .setDisplayName(LocalizedText.english("CustomDataTypeVariable")).setDataType(dataTypeId)
        .setTypeDefinition(Identifiers.BaseDataVariableType).build();
    ...
    customDataTypeVariable.addReference(new Reference(customDataTypeVariable.getNodeId(), Identifiers.Organizes,
