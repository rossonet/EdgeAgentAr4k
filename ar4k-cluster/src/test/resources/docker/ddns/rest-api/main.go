...
func main() {
    ...
    router.HandleFunc("/txt", Txt).Methods("GET")
...
func Add(w http.ResponseWriter, r *http.Request) {
    ...
    for _, domain := range response.Domains {
        result := AddRecord(domain, response.Address, response.AddrType)
...
func Txt(w http.ResponseWriter, r *http.Request) {
    ...
    for _, domain := range response.Domains {
        result := TxtRecord(domain, response.Txt, response.AddrType)
...
func TxtRecord(domain string, txt string, addrType string) string {
    log.Println(fmt.Sprintf("%s record update request: %s -> %s", addrType, domain, txt))
    ...
    w.WriteString(fmt.Sprintf("server %s\n", appConfig.Server))
    w.WriteString(fmt.Sprintf("zone %s\n", appConfig.Zone))
    w.WriteString(fmt.Sprintf("update add %s.%s %v %s %s\n", domain, appConfig.Domain, appConfig.RecordTTL, "IN TXT", txt))
    w.WriteString("send\n")
