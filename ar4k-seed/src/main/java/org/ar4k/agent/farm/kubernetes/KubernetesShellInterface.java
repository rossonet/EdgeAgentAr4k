...
public class KubernetesShellInterface extends AbstractShellHelper {
	...
	protected Availability testApiClientNull() {
		return (pc == null && client == null) ? Availability.available()
				: Availability.unavailable(
						"the Kubernetes client is already connected to " + client != null ? client.getBasePath()
								: "NaN");
	}
	...
	protected Availability testMiniKubeDashboardNull() {
		return (miniKubeDashboard == null || !miniKubeDashboard.isAlive()) ? Availability.available()
				: Availability.unavailable(
						"the Kubernetes dashboard is already running with status " + miniKubeDashboard.isAlive());
	}
	...
	public V1NodeList listK8sNodes(@ShellOption(help = "true/false", defaultValue = "true") String prettyPrint) {
		try {
			return coreApi.listNode(prettyPrint, null, null, null, null, null, null, null, null);
	...
	public V1ComponentStatusList listK8sComponentStatus(
		...
		try {
			return coreApi.listComponentStatus(true, null, null, null, null, prettyPrint, null, null, null);
		} catch (final ApiException e) {
			logger.logException(e);
			return null;
		}
	...
	public V1ConfigMapList listK8sConfigMapForAllNamespaces(
			@ShellOption(help = "true/false", defaultValue = "true") String prettyPrint) {
		try {
			return coreApi.listConfigMapForAllNamespaces(null, null, null, null, null, prettyPrint, null, null, null);
		} catch (final ApiException e) {
			logger.logException(e);
			return null;
		}
	...
	public V1PodList listK8sPodsForAllNamespaces(
			@ShellOption(help = "true/false", defaultValue = "true") String prettyPrint) {
		try {
			return coreApi.listPodForAllNamespaces(null, null, null, null, null, prettyPrint, null, null, null);
		} catch (final ApiException e) {
			logger.logException(e);
			return null;
		}
	...
	public V1ServiceList listK8sServiceForAllNamespaces(
			@ShellOption(help = "true/false", defaultValue = "true") String prettyPrint) {
		try {
			return coreApi.listServiceForAllNamespaces(null, null, null, null, null, prettyPrint, null, null, null);
		} catch (final ApiException e) {
			logger.logException(e);
			return null;
		}
	...
	public V1LimitRangeList listK8sLimitRangeForAllNamespaces(
			@ShellOption(help = "true/false", defaultValue = "true") String prettyPrint) {
		try {
			return coreApi.listLimitRangeForAllNamespaces(null, null, null, null, null, prettyPrint, null, null, null);
		} catch (final ApiException e) {
			logger.logException(e);
			return null;
		}
	...
	public V1PersistentVolumeList listK8sPersistentVolume(
			@ShellOption(help = "true/false", defaultValue = "true") String prettyPrint) {
		try {
			return coreApi.listPersistentVolume(prettyPrint, null, null, null, null, null, null, null, null);
		} catch (final ApiException e) {
			logger.logException(e);
			return null;
		}
	...
	public void addK8sNameSpace(@ShellOption(help = "namespace") String namespace) {
		try {
			final V1NamespaceBuilder nb = new V1NamespaceBuilder();
	...
	private String runK8sApply(String fileName) {
		...
		try {
			...
			final List<Object> targetList = Yaml.loadAll(confFile);
			for (final Object obj : targetList) {
				final Method m = obj.getClass().getMethod("getMetadata");
				final V1ObjectMeta metadata = (V1ObjectMeta) m.invoke(obj);
	...
	public String runKopsCommandLine(
		...
		final String[] splitCmd = command.split(" |\t");
		final String[] commandArgs = new String[splitCmd.length + 1];
		int counter = 0;
	...
	public String createK8sWithMiniKube(
		...
		result.append(runMiniKubeCommandLine(vmName, "start --cpus " + cpus + " --memory " + memory + " --disk-size="
				+ diskSize + " --vm-driver=" + driver));
		// System.out.println("Starting K8s Cluster");
	...
	public String getMiniKubeDashboard(
		...
		try {
			...
			final String[] command = {
					ConfigHelper.MINIKUBE_BINARY_PATH.replaceFirst("^~", System.getProperty("user.home")), "dashboard",
					"-p", vmName, "--url" };
		...
		try {
					...
					"apply", "all" };
			{
				...
				runCommandAndPrint(result, commandArgs1,
						new String[] {
								"KUBEFLOW_REPO=" + Paths
										.get(ConfigHelper.KUBEFLOW_DIRECTORY_PATH.replaceFirst(
												"^~", System.getProperty("user.home")) + "/kubeflow-0.4.1")
										.toAbsolutePath(),
								"PATH=" + "/usr/share/Modules/bin:/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:~/bin",
								"KUBECONFIG=" + Paths.get(
										ConfigHelper.KUBECONFIG.replaceFirst("^~", System.getProperty("user.home")))
										.toAbsolutePath() });
				Thread.sleep(1000L);
			}
			{
	...
	public String runMiniKubeCommandLine(
		...
		final String[] splitCmd = (command + " -p " + vmName).split(" |\t");
		final String[] commandArgs = new String[splitCmd.length + 1];
		int counter = 0;
	...
	private void runCommandAndPrint(StringBuilder result, String[] commandArgs, String[] env, long timeout,
		...
		try {
			...
			final Reader rin = new InputStreamReader(is, StandardCharsets.UTF_8);
			final Reader rerr = new InputStreamReader(es, StandardCharsets.UTF_8);
			final long startDate = new Date().getTime();
			while ((p.isAlive() || rin.ready() || rerr.ready())
					&& (timeout == 0 || startDate + timeout > new Date().getTime())) {
				if (rin.ready()) {
					final char ic = (char) rin.read();
					if (ic == '\n') {
						System.out.print("\r");
	...
	private void getMiniKubeBinary() throws IOException, InterruptedException {
		...
		final File miniBinary = new File(
	...
	private void getKubectlBin() throws IOException, InterruptedException {
		createBinIfNotExists();
		final File kubectlBinary = new File(
				ConfigHelper.KUBECTL_BINARY_PATH.replaceFirst("^~", System.getProperty("user.home")));
		if (!kubectlBinary.exists()) {
			final String latestKubectlVersion = HardwareHelper.readTxtFromUrl(ConfigHelper.LATEST_KUBECTL_URL);
			final String kubectlUrl = ConfigHelper.KUBECTL_URL.replace("$version",
					latestKubectlVersion.replace("\n", ""));
	...
	private void getHelmBinary() throws IOException, InterruptedException {
		...
		final File helmBinaryTgz = new File(
				ConfigHelper.HELM_TGZ_PATH.replaceFirst("^~", System.getProperty("user.home")));
		...
		final File helmBinary = new File(
				ConfigHelper.HELM_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home"))
						+ "/linux-amd64/helm");
		final File tillerBinary = new File(
				ConfigHelper.HELM_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home"))
						+ "/linux-amd64/tiller");
		if (!helmDirectory.exists()) {
			...
			logger.warn("Decompress of "
					+ ConfigHelper.HELM_TGZ_PATH.replaceFirst("^~", System.getProperty("user.home")) + " to "
					+ ConfigHelper.HELM_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home"))
					+ "/linux-amd64 completed");
			newHelm = true;
			Files.createSymbolicLink(Paths.get(
					ConfigHelper.HELM_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home")) + "/helm"),
					helmBinary.toPath().toAbsolutePath());
			Files.createSymbolicLink(Paths.get(
					ConfigHelper.HELM_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home")) + "/tiller"),
	...
	private void getKubeFlowBinary() throws IOException {
		createBinIfNotExists();
		getKsonnetBinary();
		final File kubeflowBinaryTgz = new File(
		...
		final File kubeflowDirectory = new File(
				ConfigHelper.KUBEFLOW_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home"))
						+ "/kubeflow-0.4.1");
		if (!kubeflowDirectory.exists()) {
			logger.warn("Decompress "
					+ ConfigHelper.KUBEFLOW_TGZ_PATH.replaceFirst("^~", System.getProperty("user.home")) + " to "
					+ ConfigHelper.KUBEFLOW_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home"))
					+ " PLEASE WAIT");
			HardwareHelper.extractTarGz(
					ConfigHelper.KUBEFLOW_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home")),
		...
		final File kubeflowBinary = new File(
				ConfigHelper.KUBEFLOW_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home"))
						+ "/kubeflow-0.4.1/scripts/kfctl.sh");
	...
	private void getKsonnetBinary() throws IOException {
		createBinIfNotExists();
		final File ksonnetBinaryTgz = new File(
		...
		if (!ksonnetDirectory.exists()) {
			...
			logger.warn("Decompress of "
					...
					+ " completed");
			Files.createSymbolicLink(Paths.get(
					ConfigHelper.KSONNET_DIRECTORY_PATH.replaceFirst("^~", System.getProperty("user.home")) + "/ks"),
	...
	private void getKopsBinary() throws JSONException, IOException, InterruptedException {
		createBinIfNotExists();
		final File kopsBinary = new File(
				ConfigHelper.KOPS_BINARY_PATH.replaceFirst("^~", System.getProperty("user.home")));
		if (!kopsBinary.exists()) {
			final String latestKopsVersion = HardwareHelper.readJsonFromUrl(ConfigHelper.LATEST_KOPS_URL)
					.optString("tag_name");
			final String kopsUrl = ConfigHelper.KOPS_URL.replace("$version", latestKopsVersion);
			...
			final StringBuilder awsCli = new StringBuilder();
			final String[] command = { "pip", "install", "awscli" };
