 ...
 * 
 *         Validatore per configurazione bit di stop e parità per porta seriale.
 */
public class ConventionalNotationValidator implements IParameterValidator {

	@Override
	public void validate(String name, String value) throws ParameterException {
		try {
			ConventionalNotation.valueOf(value);
		} catch (java.lang.IllegalArgumentException aa) {
			StringBuilder b = new StringBuilder();
			EnumSet.allOf(ConventionalNotation.class).forEach(v -> {
				b.append(v.toString() + " ");
			});
...
/*
 * class ConventionalNotationValuesProvider extends ValueProviderSupport {
 * 
 * private final static String[] VALUES =
 * Stream.of(ConventionalNotation.values()).map(ConventionalNotation::name)
 * .toArray(String[]::new);
 * 
