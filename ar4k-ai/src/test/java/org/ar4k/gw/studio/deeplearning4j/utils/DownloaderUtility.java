...
package org.ar4k.gw.studio.deeplearning4j.utils;
...
/**
 * Given a base url and a zipped file name downloads contents to a specified
 * directory under ~/dl4j-examples-data Will check md5 sum of downloaded file
 * <p>
 *
 * Sample Usage with an instantiation
 * DATAEXAMPLE(baseurl,"DataExamples.zip","data-dir",md5,size):
 *
 * DATAEXAMPLE.Download() & DATAEXAMPLE.Download(true) Will download
 * DataExamples.zip from baseurl/DataExamples.zip to a temp directory, Unzip it
 * to ~/dl4j-example-data/data-dir Return the string
 * "~/dl4j-example-data/data-dir/DataExamples"
 *
 * DATAEXAMPLE.Download(false) will perform the same download and unzip as above
...
public enum DownloaderUtility {
	 ...
	 *
	 * @param zipFile    Name of zipfile. Should be a zip of a single directory with
	 *                   the same name
	...
	/**
	 * Downloads a zip file from a base url to a specified directory under the
	 * user's home directory
	 *
	 * @param baseURL    URL of file
	 * @param zipFile    Name of zipfile to download from baseURL i.e
	 *                   baseURL+"/"+zipFile gives full URL
	 * @param dataFolder The folder to extract to under ~/dl4j-examples-data
	 * @param md5        of zipfile
	 * @param dataSize   of zipfile
	 */
	...
	public String Download(boolean returnSubFolder) throws Exception {
		...
		if (!new File(dataPathLocal).exists() || new File(dataPathLocal).list().length == 0) {
			...
			System.out.println("_______________________________________________________________________");
			Downloader.downloadAndExtract("files", new URL(dataURL), new File(downloadPath), new File(extractDir), MD5,
					downloadRetries);
		} else {
			System.out.println("_______________________________________________________________________");
