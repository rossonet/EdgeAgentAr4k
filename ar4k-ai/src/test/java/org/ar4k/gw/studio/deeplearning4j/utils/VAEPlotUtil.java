...
public class VAEPlotUtil {
	...
	public static void scatterPlot(List<Pair<INDArray, INDArray>> data, List<Integer> epochCounts, String title) {
		...
		JPanel panel = new ChartPanel(createChart(data.get(0).getFirst(), data.get(0).getSecond(), plotMin, plotMax,
		...
		slider.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				JPanel panel = new ChartPanel(createChart(data.get(value).getFirst(), data.get(value).getSecond(),
						plotMin, plotMax, title + " (epoch " + epochCounts.get(value) + ")"));
				if (lastPanel != null) {
					f.remove(lastPanel);
				}
				lastPanel = panel;
	...
	public static void plotData(List<INDArray> xyVsIter, INDArray labels, double axisMin, double axisMax,
			int plotFrequency) {
	...
	private static JFreeChart createChart(INDArray features, INDArray labels, double axisMin, double axisMax,
		...
		JFreeChart chart = ChartFactory.createScatterPlot(title, "X", "Y", dataset, PlotOrientation.VERTICAL, true,
				true, false);

		XYPlot plot = (XYPlot) chart.getPlot();
		plot.getRenderer().setDefaultOutlineStroke(new BasicStroke(0));
