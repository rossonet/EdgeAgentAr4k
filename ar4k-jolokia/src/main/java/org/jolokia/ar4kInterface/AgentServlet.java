 ...
 *
 * <p>
 * It uses a REST based approach which translates a GET Url into a request. See
 * the <a href="http://www.jolokia.org/reference/index.html">reference
 * documentation</a> for a detailed description of this servlet's features.
 * </p>
 *
 * @author roland@jolokia.org
 * @since Apr 18, 2009
 */
public class AgentServlet implements AbstractJolokiaInterface {
	...
	/**
	 * No argument constructor, used e.g. by an servlet descriptor when creating the
	 * servlet out of web.xml
	 */
	public AgentServlet() {
		this(null);
	}
	 ...
	 *                    should be created in the default way
	 *                    ({@link RestrictorFactory#createRestrictor(Configuration,LogHandler)})
	 */
	...
	public void init(ServletConfig pServletConfig) throws ServletException {
		...
		final Configuration config = initConfig(pServletConfig);
	...
	private void initDiscoveryMulticast(Configuration pConfig) {
		final String url = findAgentUrl(pConfig);
	...
	/**
	 * Create a log handler using this servlet's logging facility for logging. This
	 * method can be overridden to provide a custom log handler. This method is
	 * called before
	 * {@link RestrictorFactory#createRestrictor(Configuration,LogHandler)} so the
	 * log handler can already be used when building up the restrictor.
	 *
	 * @return a default log handler
	 * @param pServletConfig servlet config from where to get information to build
	 *                       up the log handler
	...
	public void doOptions(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		final Map<String, String> responseHeaders = requestHandler.handleCorsPreflightRequest(req.getHeader("Origin"),
	...
	private JSONAware handleSecurely(final ServletRequestHandler pReqHandler, final HttpServletRequest pReq,
		...
		final Subject subject = pReq.getAttribute(ConfigKey.JAAS_SUBJECT_REQUEST_ATTRIBUTE);
		if (subject != null) {
	...
	private void updateAgentDetailsIfNeeded(HttpServletRequest pReq) {
		...
		final AgentDetails details = backendManager.getAgentDetails();
		if (details.isInitRequired()) {
			synchronized (details) {
				if (details.isInitRequired()) {
					if (details.isUrlMissing()) {
						final String url = getBaseUrl(NetworkUtil.sanitizeLocalUrl(pReq.getRequestURL().toString()),
	...
	private String extractServletPath(HttpServletRequest pReq) {
		return pReq.getRequestURL().substring(0, pReq.getContextPath().length());
	}
	...
	private String getBaseUrl(String pUrl, String pServletPath) {
		...
		try {
			...
			final String host = getIpIfPossible(url.getHost());
	...
	private void setCorsHeader(HttpServletRequest pReq, HttpServletResponse pResp) {
		final String origin = requestHandler.extractCorsOrigin(pReq.getHeader("Origin"));
		if (origin != null) {
	...
	private ServletRequestHandler newPostHttpRequestHandler() {
		return new ServletRequestHandler() {
			...
			public JSONAware handleRequest(HttpServletRequest pReq, HttpServletResponse pResp) throws IOException {
				...
				final InputStream is = pReq.getInputStream();
				return requestHandler.handlePostRequest(pReq.getRequestURL(), is, encoding, getParameterMap(pReq));
			}
		};
	}
	...
	private ServletRequestHandler newGetHttpRequestHandler() {
		return new ServletRequestHandler() {
			...
			public JSONAware handleRequest(HttpServletRequest pReq, HttpServletResponse pResp) {
				return requestHandler.handleGetRequest(pReq.getRequestURL(), pReq.getPathInfo(), getParameterMap(pReq));
			}
		};
	}
	...
	// Get parameter map either directly from an Servlet 2.4 compliant
	// implementation
	...
	// Configuration from the servlet context overrides servlet parameters defined
	// in web.xml
	Configuration initConfig(ServletConfig pConfig) {
		final Configuration config = new Configuration(ConfigKey.AGENT_ID,
				NetworkUtil.getAgentId(hashCode(), "servlet"));
		// From ServletContext ....
	...
	private void sendResponse(HttpServletResponse pResp, HttpServletRequest pReq, JSONAware pJson) throws IOException {
		...
		setContentType(pResp, MimeTypeUtil.getResponseMimeType(pReq.getParameter(ConfigKey.MIME_TYPE.getKeyValue()),
	...
	private void validateCallbackIfGiven(HttpServletRequest pReq) {
		...
		if (callback != null && !MimeTypeUtil.isValidCallback(callback)) {
			throw new IllegalArgumentException(
					"Invalid callback name given, which must be a valid javascript function name");
		}
	}

	private void sendStreamingResponse(HttpServletResponse pResp, String pCallback, JSONStreamAware pJson)
			throws IOException {
		final Writer writer = new OutputStreamWriter(pResp.getOutputStream(), "UTF-8");
	...
	private void setNoCacheHeaders(HttpServletResponse pResp) {
		...
		// It happened on Tomcat that Date: was finally set *before* Expires: in the
		// final
		// answers some times which seems to be an implementation peculiarity from
		// Tomcat
	...
	private void setContentType(HttpServletResponse pResp, String pContentType) {
		...
		try {
			...
			/* Servlet 2.3 */ } catch (final UnsupportedOperationException error) {
			/* Equinox HTTP Service */ }
		if (!encodingDone) {
			// For a Servlet 2.3 container or an Equinox HTTP Service, set the charset by
			// hand
