...
/**
 * Represents an Italian verb conjugation and provide methods to change the
 * form, person and number according to a model for Italian
 */
...
public class ItalianVerbConjugation {
	...
	public enum Mode {
		INFINITIVE, GERUND, PRESENT_PARTICIPLE, PAST_PARTICIPLE, INDICATIVE_PRESENT, INDICATIVE_IMPERFECT,
		INDICATIVE_PAST_HISTORIC, INDICATIVE_FUTURE, CONDITIONAL_PRESENT, SUBJUNCTIVE_PRESENT, SUBJUNCTIVE_IMPERFECT,
		IMPERATIVE
	}
	...
	/**
	 * Instantiates a verb conjugation object starting from the string
	 * representation from the toStringRepresentation method
	 *
	...
	public String getConjugated(boolean guess) throws ConjugationException {
		...
		if (mode == Mode.GERUND || mode == Mode.PAST_PARTICIPLE || mode == Mode.PRESENT_PARTICIPLE
				|| mode == Mode.INFINITIVE) {
			...
			if (this.conjugated == null) {
				...
				if (this.infinitive.length() < 4) {
					throw new ConjugationException(
							"the verb '" + infinitive + "' is not really a verb and cannot be conjugated");
				}
				String defaultSuffix = ItalianVerbConjugation.defaultSuffixes
						.get(this.infinitive.substring(this.infinitive.length() - 3, this.infinitive.length()) + ","
								+ this.getMode());
				if (defaultSuffix == null)
					throw new ConjugationException(
							"conjugation unknown for the verb " + infinitive + " and mode " + this.getMode());
			...
			if (this.person < 1 || this.person > 3)
				throw new ConjugationException("person " + this.person
						+ " not valid, have to be 1,2 or 3 since the mode is " + this.getMode());
			// imperative doesn't allow for first singular person
			if (this.person == 1 && this.number == 's' && this.getMode().equals("imperative"))
				throw new ConjugationException(
						"the Italian imperative form does not have the first singular person (you can't give orders to yourself!)");

			// is the number a valid one?
			if (this.number != 's' && this.number != 'p')
				throw new ConjugationException("number " + this.person
						+ " not valid, have to be 's' or 'p' since the mode is " + this.getMode());
			...
			if (this.conjugated == null) {
				...
				String searchFor = this.infinitive.substring(this.infinitive.length() - 3, this.infinitive.length())
						+ "," + this.getMode() + "," + this.person + "," + this.number;
				String defaultSuffix = defaultSuffixes.get(searchFor);
				if (defaultSuffix == null)
					throw new ConjugationException("conjugation unknown for the verb " + infinitive + " and mode "
							+ this.getMode() + " person " + this.person + " number " + this.number + " (search string:'"
							+ searchFor + "')");
				String root = this.infinitive.substring(0, this.infinitive.length() - 3);
				// if the infinitive contains "ci" or "gi" before the ending, change the
				// conjugations
				// for example "mangiare" becomes "mangerò" not "mangierò"
				if ((root.endsWith("ci") || root.endsWith("gi")) && ((defaultSuffix.startsWith("e")
						|| (defaultSuffix.startsWith("i")) && !root.endsWith("cci") && !root.endsWith("ggi")))) {
	...
	public String toStringRepresentation(String delim) {
		return this.conjugated + delim + this.infinitive + delim + this.getMode() + delim + this.number + delim
				+ this.person;
	}

	/**
	 * Returns the mode and the time of the conjugation, for example "indicative
	 * past historic"
	 *
	...
	public static Set<ItalianVerbConjugation> guessVerb(String word, ItalianModel italianModel) {
	...
	public static boolean isImpersonalMode(String mode2) {
		for (Entry<Mode, String> mr : modeRepresentations.entrySet()) {
			if (mr.getValue().equals(mode2)) {
				Mode mode = mr.getKey();
				return mode == Mode.GERUND || mode == Mode.PAST_PARTICIPLE || mode == Mode.PRESENT_PARTICIPLE
						|| mode == Mode.INFINITIVE;
			}
		}
	...
	public String toJSON() {
		return "{'infinitive':'" + this.infinitive + "'" + ",'mode':'" + this.getMode() + "'"
